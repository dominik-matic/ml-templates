...
class Trainer:
	...
	def _save_snapshot(self, epoch):
		if self.verbose:
			tqdm.write(f"Saving snapshot to {self.snapshot_path}...")
		...
		if self.verbose: # maybe remove these DONEs?
			tqdm.write("DONE.")
	...
	def _load_snapshot(self, load_path):
		...
		snapshot = torch.load(load_path, map_location=loc)
	...
	def train(self, n_epochs):
		...
		for i in (pbar := tqdm(range(self.current_epoch, n_epochs), desc=f"E{self.current_epoch} TL={self.train_losses[-1]:.8f} VL={self.valid_losses[-1]:.8f}")):
			...
			l = self._valid_epoch(self.valid_data)
			self.valid_losses.append(l)
			if self.device == 0 and (self.lowest_valid_loss is None or l < self.lowest_valid_loss):
				self.lowest_valid_loss = l
				self._save_model(i)
			if self.device == 0 and (i % self.epoch_save_interval == 0):
